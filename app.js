...
function hideOldInput(selector) {
  ...
  // Hide the input and its most likely field wrapper.
  const wrapper =
    el.closest(".field") ||
...
function hideOldSections() {
  // v1.4 deliberately uses natural language instead of separate
  // budget / priority / traveller controls.
  hideOldInput("#budget");
  hideOldInput("#people");
  ...
  if (chips) {
    const wrapper =
      chips.closest(".field") ||
      chips.closest(".form-group") ||
      chips.parentElement;
    if (wrapper) wrapper.style.display = "none";
    else chips.style.display = "none";
  }

  // Remove any visible old headings associated with those controls.
  qa("label, h3, h4, p").forEach(el => {
