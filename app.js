...
function activityHtml(activities, people) {
  ...
  return `
    <section class="option">
      <h2>🎟️ Aktivitet som DestyPoint hittade</h2>
      ${activities.map(a => `
...
function init() {
  ...
  if (form) form.addEventListener("submit", event => { event.preventDefault(); runAgent(); });

  document.querySelectorAll(".inspiration").forEach(btn => {
  ...
  const go = q("#go");
  if (go) go.addEventListener("click", event => { event.preventDefault(); runAgent(); });
