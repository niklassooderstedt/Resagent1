/* ResAgent v1.4
   Clean build based on the actual v1.3 index.html/style.css.
   The form contains only description + dates.
*/
...
function parseTripType(text) {
  ...
  if (/familj|barn|barnfamilj|familjeresa|zoo|djurpark|nojespark|liseberg|grona lund|astrid lindgrens|high chaparral|high chaparall/.test(t)) return "familj";
...
function scoreDestination(destination, tripType, priorities, budget) {
  ...
  if (destination === "Stockholm") {
    score = tripType === "spa" ? 94 : tripType === "weekend" ? 92 : tripType === "familj" ? 91 : 88;
  }
...
function findActivities(text, destination, tripType) {
  ...
  if (tripType === "familj" && destination === "Stockholm") {
    return ACTIVITIES.filter(a => ["Gröna Lund","Skansen"].includes(a.title));
  }
  if (tripType === "familj" && destination === "Göteborg") {
    return ACTIVITIES.filter(a => a.title === "Liseberg");
  }
...
function isSwedenTrip(destination, text) {
  const t = normalize(text);
  const SwedishTerms = [
    "sverige","stockholm","goteborg","malmo","uppsala","visby","orebro",
    "vasteras","linkoping","norrkoping","jonkoping","kalmar","halmstad",
    "helsingborg","kiruna","are","gotland","skane","dalarna"
  ];

  return SwedishTerms.some(x => t.includes(x)) ||
    ["Stockholm","Göteborg","Malmö","Uppsala","Visby","Örebro","Västerås",
     "Linköping","Norrköping","Jönköping","Kalmar","Halmstad","Helsingborg",
     "Kiruna","Åre","Gotland","Skåne","Dalarna"].includes(destination);
}
...
function transportEstimate(origin, destination, people, text) {
  ...
  const longTrip =
    /stockholm.*goteborg|goteborg.*stockholm|stockholm.*malmo|malmo.*stockholm|kiruna/.test(t);

  const trainPerPerson = longTrip ? 650 : 420;
  const train = trainPerPerson * people * 2;

  // Schablon för bil tur/retur. Inte ett aktuellt drivmedelspris.
  const car = longTrip ? 2200 : 1300;

  const boat = /visby|gotland|farja|bat|skargard/.test(t)
    ? 900 * people
    : null;
    ...
    <section class="option">
      ...
      ${activities.map(a => `
        <div class="good" style="margin-top:12px">
          ...
          <a target="_blank" rel="noopener" href="${a.link}">🎟️ Se aktuella biljetter</a>
        </div>
      `).join("")}
    </section>
  `;
...
function runAgent() {
  ...
  const html = `
    ...
    <article class="option">
      ...
      <h2>${destination}</h2>
      <p class="muted">ResAgent har tolkat din beskrivning och anpassat rekommendationen efter dina önskemål.</p>

      <div class="break">
        <div><b>Boende</b>${money(budgetForEstimate * 0.43)}</div>
        <div><b>Transport</b>${money(budgetForEstimate * 0.22)}</div>
        <div><b>Mat</b>${money(budgetForEstimate * 0.15)}</div>
        <div><b>Aktiviteter</b>${money(budgetForEstimate * 0.20)}</div>
      </div>
...
function init() {
  ...
  const go = q("#go");
  if (go) go.addEventListener("click", runAgent);
